...
<html>
	...
	function proChange(objVal){
	 ...
	 var url = "${ctx}/productseriemanagement.do?method=getMaxSerNobyTypeId&typeid="+objVal;
	 XMLHttpReq.open("GET",url,true);
	 XMLHttpReq.send(null);
	}
//解析XML信息，添加城市
	function parseXML(xmlDoc){
	var noo = document.getElementById("se").value;
	if(noo!=""){
	no = xmlDoc.getElementsByTagName("maxno")[0].firstChild.data;
	document.getElementById("maxno1").innerHTML=no;
	document.getElementById("maxno").value=no;
	}else{
	document.getElementById("maxno1").innerHTML="";
	document.getElementById("maxno").value="";
	}
	creat();
	}
		var formId = 0;
		//控件名
		var checknNames = [ "productserie.product_type_id1", "productserie.name1"];
		//提示语
		var descriptions = [ "产品分类", "系列名称"];
		...
		function creat(){
		 var serieno = document.getElementById("maxno").value;
		  var noo = document.getElementById("se").value;
		  var sss;
		  if(noo!=""){
		  var no = noo.split(",");
			...
			<tr>
				...
				<td align="center">
					...
					<html:form action="/productseriemanagement" method="post">
						<input type="hidden" name="method" value="addProductSerie" />
						<table width="100%" border="0" cellpadding="0" cellspacing="0"
							class="biao3">
							<tr>
								...
								<td class="td_02">
									<label>
										<select name="productserie.product_type_id1" id="se"
											onchange="proChange(this.value);" style=" width:126px">
											<option value="" selected="selected">
												--请选择--
											</option>
											<logic:present name="ptypename">
												<logic:iterate id="ptypenames" name="ptypename">
													<option value="${ptypenames.idNo}">
														${ptypenames.name}
													</option>
												</logic:iterate>
											</logic:present>
								...
								<td class="td_02">
									<input type="text" name="productserie.name1" maxlength="12"
										id="name" style="width:120px;" />
									<input type="hidden" name="max" id="maxno" />
							...
							<tr>
								...
								<td class="td_02" height="18" id="maxno1">
								</td>
								<td class="td_01">
									产品系列编码
								</td>
