...
//解析XML信息，添加城市
	...
	document.getElementById("supplier.area").value=areaName;
		...
		function shan(){
		 if(myArray.length==0){
				...
				window.location = '${ctx}/customer.do?method=deleteSupplier&id='+myArray.join(',');
				//document.forms[0].action ='${ctx}/customer.do?method=deleteSupplier&id='+myArray.join(',');
		...
		function fMainListToggle(obj){
			...
			for(var i=0;i<tmp.length;i++){
				...
				if(tmp[i].className == 'mo_title_r1 btn_all_tiny_img'){
					domImg = tmp[i];
				}
			}  
			...
			var formId = 0;
				...
				//提示语
				var descriptions = ["客户名称","区域","省/直辖市","市"];
	...
	<body onload="checks();">
		<form action="customer.do">
			...
			<input type="hidden" id="oldPageByCustomer" value="cutPage" />
								...
								<table width="100%" border="0" cellpadding="0" cellspacing="0"
									...
									<tr>
										<td colspan="4" align="center" height="30px">
											<a href=#><img src="${ctx}/images/btn_JianSuo.gif" onclick="checkSupplier();" /></a>
										</td>
									</tr>
								</table>
							</div>
							...
							<logic:present name="supplierList">
								<logic:iterate id="supplier" name="supplierList"
									...
									<tr>
										<c:if test="${roleId==2}">
										<td>
											&nbsp;<input name="ss" type="checkbox" value="${supplier.id }" onclick="javascript:removeBefore(this);" />
										</td>
										</c:if>
										...
										<td nowrap="nowrap" width="70px">
											<a href="javascript:window.location = '${ctx}/customer.do?method=supplierLook&id=${supplier.id}';">查看</a><c:if test="${roleId==2}">
											<a href="javascript:window.location = '${ctx}/customer.do?method=showSupplierUpdate&id=${supplier.id}';">修改</a>
											</c:if>
										</td>
									</tr>
								</logic:iterate>
						...
						<br />
							...
							<table border="0" cellpadding="0" cellspacing="0" width="100%"
								...
								<tr>
									...
									<td width="100px">
										<a
											href="javascript:window.location = '${ctx}/customer.do?method=showSupplierAdd';"><img
