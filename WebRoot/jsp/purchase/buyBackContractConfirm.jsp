...
<c:if test="${buyBackContract.status!=9}">
	<script>
	location="${ctx}/buyBackContractList.do";
	</script>
</c:if>
<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
<title>采购退货合同确认</title>
        ...
        <script type="text/javascript">
        	function contractConfirm(subType,buyBackContractId){
        		var companyContractCode=document.getElementById("companyContractCode").value;
        		companyContractCode=encodeURI(companyContractCode);
        		var companyContract=document.getElementById("companyContractCode");
        		if(buyBackContractId==null||buyBackContractId==""){
        			alert("采购退货合同编号不存在！");
        			return ;
        		}
        		if(subType=="1"){
        			if(companyContractCode==null||companyContractCode==""){
	        			$(companyContract).addClass("invalid");
	        			$(companyContract).focus();
	        			alert("公司合同号不能为空");
	        			return;
        			}
        			var date=new Date();
        			$.getJSON("${ctx}/isExistsBuyBackContractCode.do?companyContractCode="+companyContractCode+"&date="+date.getSeconds() , function(returndata){
        				if(returndata==null){
        					$(companyContract).addClass("invalid");
        					$(companyContract).focus();
        					alert("公司合同号不能为空");
        				 ...
        				 if(confirm("警告：公司合同号填写后不可更改!请再次确认！")){
        					//location="${ctx}/buyBackContractConfirm.do?subType="+subType+"&buyBackContractId="+buyBackContractId+"&companyContractCode="+companyContractCode;
        					$("#subType").val(subType);
        					$("#buyBackContractConfirmForm").submit();
        				}
        				}
        			});
        		}else if(subType=="0"){
        				if(confirm("确认要取消合同吗？")){
        			 location="${ctx}/buyBackContractConfirm.do?subType="+subType+"&buyBackContractId="+buyBackContractId;
        			}
        		}
        	}
        </script>
...
<table width="100%" border="0" cellpadding="0" cellspacing="0">
  <tr>
    ...
    <td class="ye_header_center"><img src="${ctx}/images/main_jt.jpg" />&nbsp; 当前位置： 采购管理 &gt;&gt; 采购退货合同管理 &gt;&gt; 采购退货合同确认</td>
    ...
    <td><br />
        ...
        <tr id="tr_MT">
		 ...
		 <tr>
		  ...
		  <td colspan="3"  class="td_02">&nbsp;
		  	<html:form action="buyBackContractConfirm" styleId="buyBackContractConfirmForm" method="post">
			<input type="text" name="companyContractCode" id="companyContractCode" maxLength="20" />
			<input type="hidden" name="subType" id="subType" value=""/>
			<input type="hidden" name="buyBackContractId" id="buyBackContractId" value="${buyBackContract.buyBackContractId}"/>
			</html:form>
          </td>
        </tr>
      </table>
      ...
      <table width="100%" border="0" cellpadding="0" cellspacing="0" class="biao3">
        <tr>
          ...
          <td class="td_04" >${tf:replaceBr(buyBackContract.text)}
          </td>
        </tr>
      </table>
		...
		<table border="0" cellpadding="0" cellspacing="0" align="center" width="460">
			<c:if test="${buyBackContract.templateType==1}">
				...
				<div style="text-align:center;">
				 <a href="javascript:contractConfirm('1','${buyBackContract.buyBackContractId}')" id="decide"><img src="${ctx}/images/btnSX.gif" width="65" height="20" /></a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; 
				 <a href="javascript:contractConfirm('0','${buyBackContract.buyBackContractId}')" id="cancel"><img src="${ctx}/images/btnQXHT.gif" width="88" height="20" /></a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; 
				 <a href="#" onclick="javascript:history.back()"><img src="${ctx}/images/btnBack.gif" /></a> </div> 
					</td>
    <td></td>
  </tr>
			</table>
