...
var leCurrentDateDisable=false;//小于当前日期的时间是否禁用； 默认不禁用
var h_today=false;
var h_date=new Date().getDate();
var h_year=new Date().getFullYear();
var h_month=new Date().getMonth()+ 1;
...
function calendarMinToday(){
	leCurrentDateDisable=true;
	calendar();
}
function calendarMinTomorrow(){
leCurrentDateDisable=true;
h_today=true;
calendar();
};
function calendar() //主调函数
{
    ...
    var cw = WebCalendar.calendar.scrollWidth, ch = WebCalendar.calendar.clientHeight;
    var dw = document.body.scrollWidth, dl = document.body.scrollLeft, dt = document.body.scrollTop;
...
function dayMouseOver()
{
  	...
  	if(leCurrentDateDisable){
  		
		if(this.style.color.toLowerCase()!=WebCalendar.wordDark.toLowerCase()){
		    this.className = "over";
    ...
    for(var i=0; i<WebCalendar.dayShow; i++)    //这个循环是根据源数组写到日历里显示
    {
        ...
        WebCalendar.dayObj[i].style.color  = WebCalendar.wordColor;
         if(leCurrentDateDisable){
	        ...
	        var gtyear=y==(new Date().getFullYear())
	        var month=m<(new Date().getMonth()+1);
	        var gtmonth=m==(new Date().getMonth()+1);
	        var date=parseInt(WebCalendar.day[i], 10)<(new Date().getDate())
	        if(gtyear&&gtmonth&&date){
...
function returnDate() //根据日期格式等返回用户选定的日期
{
    if(WebCalendar.objExport)
    {
        ...
        if(leCurrentDateDisable){
	       var b=true;
	       if((new Date().getFullYear())>a[2]){//判断选择的年份是否小于当前年份
	       		//alert("选择的年份不能小于"+(new Date().getFullYear())+"年");	
	       		return false;
	       }
	       if((new Date().getFullYear())==a[2]&&(new Date().getMonth()+ 1)>a[1]){//在麻醉年份正确的情况下，判断选择的月份是否小于当前的月份
	       		//alert("选择的月份不能小于"+(new Date().getMonth()+ 1)+"月");	
	       		return false;
	       }
	       var prh_date=new Date().getDate();
	       if(h_today&&(new Date().getFullYear()==a[2]&&(new Date().getMonth()+ 1)==a[1])){
	       	prh_date=prh_date+1;
	       }
	       if(prh_date>a[0]&&(new Date().getFullYear()==a[2]&&(new Date().getMonth()+ 1)==a[1])){//在满足年份和月份正确的情况下，判断选择的日期是否小于当前的日期
	       		//alert("选择的日期不能小于"+(new Date().getDate())+"号");	
	       		return false;
	       }
       }
